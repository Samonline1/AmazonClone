...
<div className="flex flex-col relative h-screen z-50 bg-white text-gray-900 p-10 w-full gap-5">
  ...
  <a className="text-gray-500" href={`/category/toy`}>Toy</a>
  <a className="text-gray-500" href={`/category/toy`}>Toy</a>
  <a className="text-gray-500" href={`/category/toy`}>Toy</a>
  <b className="text-lg">Shop by category</b>
  {categories.splice(5, 10).map((category)=>(
  <a key={category} href={`/category/${category}`} className="text-gray-500">
  {category.replace(/-/g, "        " , ">" )}
</a>
))}
</div>
...
<p className="text-white" onClick={()=> setShow(true)}>All</p>
{categories.splice(0, 5).map((category)=>(
  <a key={category} href={`/category/${category}`} className="text-white">
  {category.replace(/-/g, "        ")}
</a>
))}
        </nav>
